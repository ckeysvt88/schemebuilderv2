...
const DOWN_DIST_MODS = {
  ...
  '1_short':  { run: +8,  pass: -5,  pressure: -8,  hybrid: +3  },
  '1_medium': { run: +5,  pass: 0,   pressure: -5,  hybrid: +5  },
  '1_long':   { run: 0,   pass: +5,  pressure: +3,  hybrid: +3  },

  // 2nd down — context-driven
  '2_short':  { run: +10, pass: -8,  pressure: -5,  hybrid: +5  },
  '2_medium': { run: +3,  pass: +3,  pressure: +5,  hybrid: +5  },
...
export function applyDownDistance(scored, down, distance) {
  ...
  const mods = DOWN_DIST_MODS[key];
  if (!mods) return scored;

  return [...scored]
    .map(fm => {
      const delta = mods[fm.priority] || 0;
      return { ...fm, sc: Math.max(0, Math.min(100, fm.sc + delta)), ddDelta: delta };
    })
    .sort((a, b) => b.sc - a.sc);
}
