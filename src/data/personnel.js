...
export const PMAP = {
  ...
  p02:   { label: "02p (0 RB, 2 TE, 3 WR — Empty TE)", priority: "DOLLAR, not Dime. Dollar carries 2 CB and FOUR SAFETIES — safeties cover and tackle tight ends; corners do not. Two TEs with no back is exactly what Dollar exists for. No RB = no run threat and no blitz pickup. Two TEs create twin seam threats that LBs cannot handle — you need DBs in the middle. Identify which TE is the seam receiver and which is blocking before the snap. MLB must wall the seam if he stays on the field; otherwise use a safety over the top.", avoid: "Any base formation with LBs covering TEs in space, 4-3, 3-4, 5-2 (you have no WRs to guard against but every TE and WR route wins vs. a LB)", blitzNote: "High (28–42%). 5 OL with no RB pickup — overload one side from the edge. If both TEs stay in protection (7-man protection), reduce blitz and play Cover 2 behind." },
  ...
  p12:   { label: "12p (1 RB, 2 TE)",      priority: "Base or Nickel+LB. Two TEs threaten run AND seam simultaneously.", avoid: "Dime (need LBs for run), Dollar (hybrid LBs vs physical TEs = mismatch)", blitzNote: "Low-moderate (15–22%). TE hot routes beat blitz. Coverage wins this matchup." },
  p13:   { label: "13p (1 RB, 3 TE)",      priority: "Max box defenders immediately. Three TEs can block or release — identify the pass-catching TE pre-snap. Never go Nickel — you need LBs to account for the extra gap.", avoid: "Nickel, Dime, Dollar (massively outnumbered at the point of attack)", blitzNote: "Very low (10–15%). Three TEs in protection collapses blitz lanes. Run defense wins this." },
  ...
  p21:   { label: "21p (2 RB, 1 TE)",      priority: "Base personnel. MLB must take the FB — frees everyone else for the HB.", avoid: "Dime, Dollar (not enough run defenders)", blitzNote: "Low (14–20%). FB blocks blitzers naturally. Coverage gaps more dangerous." },
...
export const FAMILY_ADJUSTMENTS = {
  // ── 00 Personnel ─────────────────────────────────────────────────────────
  p00_gun:    { extra:"00p Gun — 5 WRs, no RB, no TE. Go Dime Rush or Dollar Sugar 3-2 immediately. No run threat whatsoever — LBs are automatic mismatches on WRs. Blitz freely: their 5 OL cannot account for any extra rusher without a back.", bias:["Dime Rush","Dollar Sugar 3-2","Dime Normal"] },
  p00_trips:  { extra:"00p Trips — 5 WRs with a trips surface. The 3-on-2 overload is every snap, every formation. Dollar Sugar 3-2 gives 6 DBs to match all 5 WRs plus an apex for the flood. Cover 6 to the trips side is the only answer — Cover 3 is outnumbered 3-to-2.", bias:["Dollar Sugar 3-2","3-3-5 Split","Nickel Wide"] },
  p00_motion: { extra:"00p with motion shifts the 5-WR surface last second. Hold your 2-high shell until motion settles, then rotate. Dime gives you 6 DBs to match any alignment they shift to. Never press — 5 WRs with motion will rub defenders into each other.", bias:["Dime Normal","Dollar Sugar 3-2","Nickel 3-3 Dbl Mug"] },
  // ── 01 Personnel ─────────────────────────────────────────────────────────
  p01_gun:    { extra:"01p Gun — no RB, 1 TE, 4 WRs. Go Dime immediately — this is nearly identical to 00p but the TE creates a specific seam or slot mismatch. Identify the TE alignment pre-snap: slot TE = put your best slot DB on him, not a LB. Blitz freely from both edges — OL has no RB help.", bias:["Dime Rush","Dollar Sugar 3-2","Dime Normal"] },
  p01_trips:  { extra:"01p Trips — near-empty with a trips surface and a TE as the 4th receiver to one side. Dollar Sugar 3-2 gives 6 DBs to match all 5 threats plus the flood overload. Cover 6 to the trips side; hold the rotation until the TE declares his route — seam or flat.", bias:["Dollar Sugar 3-2","Dime Normal","3-3-5 Split"] },
  // ── 02 Personnel ─────────────────────────────────────────────────────────
  p02_gun:    { extra:"02p Gun — no RB, 2 TEs, 3 WRs. Sub into Dime or Dollar Sugar 3-2. No run threat from the backfield — the OL has zero pickup help on blitzes. Identify both TE alignments: a TE in the slot or released to the seam beats any LB in coverage. MLB must wall both seams or use safety help over the top of each TE.", bias:["Dollar Sugar 3-2","Dime Normal","Nickel 3-3 Dbl Mug"] },
  p02_trips:  { extra:"02p Trips — empty TE spread with a trips flood and a TE as extra mismatch. Cover 6 to the trips side; identify which TE is the seam threat and which is in-line. Dollar gives 6 DBs to match every route — the only correct answer when 5 eligible receivers are on one side of the field.", bias:["Dollar Sugar 3-2","3-3-5 Split","Dime Normal"] },
  // ── 10 Personnel ─────────────────────────────────────────────────────────
  ...
  // ── 20 Personnel ─────────────────────────────────────────────────────────
  p20_gun:    { extra:"20p Gun — 2 RBs and 3 WRs from spread. Sub into Nickel but keep one LB active — the 2nd RB is live as a runner or receiver out of the flat. CBs at 7 yards, never press. SS walks to the flat to eliminate the 2nd RB checkdown.", bias:["Nickel 3-3 Over Jack","Nickel Wide","3-3-5 Stack"] },
  p20_pistol: { extra:"20p Pistol — dual-RB pistol means outside zone and power are equally live. Nickel keeps coverage on 3 WRs while the LB accounts for the 2nd back. DE rule: contain the perimeter every snap — do not crash on zone fakes with 2 RBs in the backfield.", bias:["Nickel 3-3 Over Jack","4-3 Under","3-3-5 Over Flex"] },
