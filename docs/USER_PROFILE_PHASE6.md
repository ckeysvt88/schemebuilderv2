...
The game plan now has a compact **My Defense** control. It stores two preferences on the current device:
...
Malformed or obsolete saved values fall back to `Middle defender` and `Balanced`. The profile is separate from the opponent scout and is not exported as an opponent tendency.
